...
function calculateTile({ areaInputMode, length, width, area, tileLength, tileWidth, tilesPerPack }) {
  validatePositiveNumber(tileLength, "Tile length");
  validatePositiveNumber(tileWidth, "Tile width");
  validatePositiveNumber(tilesPerPack, "Tiles per pack");

  let totalArea;

  if (areaInputMode === "squareFeet") {
    validatePositiveNumber(area, "Area");
    totalArea = area;
  } else {
    validatePositiveNumber(length, "Area length");
    validatePositiveNumber(width, "Area width");
    totalArea = length * width;
  }

  const tileArea = (tileLength * tileWidth) / 144;
  const tilesRequired = Math.ceil(totalArea / tileArea);
  const packsRequired = Math.ceil(tilesRequired / tilesPerPack);

  return {
    totalArea: round(totalArea, 2),
    tilesRequired,
    packsRequired,
    tileAreaLabel: `Each tile covers about ${round(tileArea, 2)} square feet.`,
  };
}

function calculateConcrete({ length, width, height }) {
  ...
  return {
    ...
    concreteLabel: "Concrete is commonly ordered in cubic yards in the US and cubic meters in many metric regions.",
  };
}
...
function calculateSquareFootage({ length, width }) {
  ...
  return {
    ...
    areaLabel: `A ${length} foot by ${width} foot area covers ${round(squareFeet, 2)} square feet.`,
  };
}
